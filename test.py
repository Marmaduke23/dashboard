...
def crear_grafico_horometro(mes,ano,dataset,columna,titulo):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='skyblue',#['red' if x else 'skyblue' for x in datos_mes_completos['sin_dato']],  # Colores
            name='Diferencia Horómetro'
        ))
        # Agregar línea roja a las 24 horas
        fig.add_trace(go.Scatter(
            x=datos_mes_completos.index,  # Mismos días del mes
            y=[24] * len(datos_mes_completos),  # Valor constante de 24
            mode='lines',
            line=dict(color='red', dash='dash'),  # Línea roja punteada
            name='24 Horas'  # Nombre en la leyenda
        ))
        ...
        fig.update_layout(
            ...
            yaxis=dict(tickformat=".2f",range=[0,30]),  # Formato de las diferencias
...
app.layout = dbc.Container(
    [
        ...
        html.H1("PLANTA SAN JOSE DE CHUCHUNCO", style={'text-align': 'center'}),
                ...
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            ...
                            value=df['Dif registro'].iloc[-1].year,
                            clearable=False,
                            style={"width": "150px"},
                        ),
                    ],
                ...
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            ...
                            value=df['Dif registro'].iloc[-1].month,
                            clearable=False,
                            style={"width": "200px"},
                        ),
                    ],
...
def actualizar_dashboard(n_intervals):
    try:
        ...
        #Estanques
        figura_estanque_3000= generar_grafico_estanque(3000, df['Elevado 3000 m3 - Nivel de Estanque - m3'].iloc[-1],'elevado 3000','m3',1200,600)#cambiar volumen por altura
        figura_estanque_6000= generar_grafico_estanque(9, df['Semi Enterrado 6000 m3 - Nivel de Estanque - m'].iloc[-1],'semi enterrado 6000','m',3.5,1.7)#cambiar volumen por altura
        #Botones Booster
        ...
        nivel_6000_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Semi Enterrado 6000 m3 - Nivel de Estanque - m','Nivel de Estanque Semienterrado - m',[0,9])
        #Historicos pozos
        caudal_1a_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Pozo 1A - Caudal l/s','Caudal Pozo 1A',[0,100],width=800,height=500)
        caudal_2a_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Pozo 2A - Caudal l/s','Caudal Pozo 2A',[0,100],width=800,height=500)
        caudal_3a_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Pozo 3A - Caudal l/s','Caudal Pozo 3A',[0,100],width=800,height=500)
        caudal_4a_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Pozo 4A - Caudal l/s','Caudal Pozo 4A',[0,100],width=800,height=500)
        caudal_5_hist=crear_grafico_dia(df_actualizado['Dif registro'].iloc[-1],df_actualizado,'Pozo 5 - Caudal l/s','Caudal Pozo 5',[0,100],width=800,height=500)
...
def actualizar_grafico(fecha):
    ...
    fecha = pd.to_datetime(fecha) if fecha else None
    ...
    figura_nivel_6000=crear_grafico_dia(fecha,df,'Semi Enterrado 6000 m3 - Nivel de Estanque - m','Nivel de Estanque Semienterrado - m',[0,9])
    figura_1a_hist=crear_grafico_dia(fecha,df,'Pozo 1A - Caudal l/s','Caudal Pozo 1A',[0,100],width=800,height=500)
    figura_2a_hist=crear_grafico_dia(fecha,df,'Pozo 2A - Caudal l/s','Caudal Pozo 2A',[0,100],width=800,height=500)
    figura_3a_hist=crear_grafico_dia(fecha,df,'Pozo 3A - Caudal l/s','Caudal Pozo 3A',[0,100],width=800,height=500)
    figura_4a_hist=crear_grafico_dia(fecha,df,'Pozo 4A - Caudal l/s','Caudal Pozo 4A',[0,100],width=800,height=500)
    figura_5_hist=crear_grafico_dia(fecha,df,'Pozo 5 - Caudal l/s','Caudal Pozo 5',[0,100],width=800,height=500)
